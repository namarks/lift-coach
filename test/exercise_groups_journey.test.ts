...
beforeAll(async () => {
  ...
  await mcp('update_plan', { name: fixture.name, days: [{ name: fixture.day_name, exercises: fixture.slots.map((slot) => ({
  ...
  for (const [index, group] of fixture.groups.entries()) {
    const result = await api(`/days/${plan.days[0].id}/groups`, 'PUT', {
      group_id: groupIDs[index], expected_version: plan.version,
      exercises: group.member_indices.map((i) => plan.days[0].exercises[i].id),
...
describe('shared simulator and real-D1 superset workout', () => {
  it('reads the member-authored warm-up and working group identically through state and coach', async () => {
    ...
    expect(coach.days[0].exercises.map((slot: any) => slot.group_label)).toEqual(['A1', 'A2', 'B1', 'B2']);
    for (const [index, expected] of fixture.slots.entries()) {
      const member = state.body.plan.days[0].exercises[index];
      ...
      const { group_label: _label, ...canonical } = coach.days[0].exercises[index];
  ...
  describe('recorded alternating rounds', () => {
    ...
    beforeAll(async () => {
      const created = await api('/sessions', 'POST', {
        date: fixture.date, day_template_id: plan.days[0].id, expected_attempt: 0,
      });
      expect(created.status).toBe(201);
      session = created.body;
      for (const index of fixture.execution_indices) {
        const slot = plan.days[0].exercises[index];
    ...
    it('retains each physical set on its own slot with unchanged warm-up classification', async () => {
      expect(logs.map((set) => set.template_exercise_id))
        .toEqual(fixture.execution_indices.map((index) => plan.days[0].exercises[index].id));
