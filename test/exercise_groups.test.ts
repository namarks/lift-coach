...
import { addTemplateExercise, adjustToday, clearGroup, dedupeDayOrderIndexes, deleteDayTemplate,
...
async function fixture() {
  ...
  const result = await updatePlanTree(env.DB, userId, { name: 'Groups', days: [
  ...
  const tree = result.plan; const day = tree.days[0]!;
...
function rebuild(tree: PlanTree) {
  return { name: tree.name, expected_version: tree.version, days: tree.days.map((day) => ({
...
describe('exercise groups service contract', () => {
  it('commits the group, its original rests, attribution and canonical snapshots together', async () => {
    ...
    expect(tree.days[0]!.exercises.slice(0, 2).map((slot) => [slot.group_id, slot.group_rest_seconds,
  ...
  it('does not restore old membership when replayed after a rewrite or full rebuild', async () => {
    ...
    expect(await setGroup(env.DB, f.userId, f.day.id, f.groupId, f.ids.slice(0, 2), firstOptions, attribution))
      .toEqual({ ...first, replayed: true });
    const tree = await current(f.userId);
    expect(tree.days[0]!.exercises.map((slot) => slot.group_id)).toEqual([null, f.groupId, f.groupId, null]);
  ...
  it('clears once, retains ordinary rest, and safely replays old clears after regrouping', async () => {
    ...
    expect(await counts(f.userId)).toEqual(before);
    expect((await current(f.userId)).days[0]!.exercises.slice(0, 2).map((slot) => [slot.group_id, slot.rest_seconds]))
    ...
    expect(await clearGroup(env.DB, f.userId, f.groupId, version)).toEqual({ ...cleared, replayed: true });
    expect((await current(f.userId)).days[0]!.exercises[0]!.group_id).toBe(f.groupId);
  });
  ...
  it('moves the complete block and updates every member count atomically', async () => {
    ...
    const slots = (await current(f.userId)).days[0]!.exercises;
  ...
  it('normalizes a deleted group member and validates every write even for unrelated fields', async () => {
    ...
    expect((await current(f.userId)).days[0]!.exercises.slice(0, 2).map((slot) => slot.group_id)).toEqual([f.groupId, f.groupId]);
    ...
    expect((await current(f.userId)).days[0]!.exercises[0]).toMatchObject({ id: f.ids[2], group_id: null,
  ...
  it('preserves group serialization through rebuilds and rejects a split rebuild', async () => {
    ...
    expect(tree.days[0]!.exercises[0]).toMatchObject({ group_id: f.groupId, group_rest_seconds: 30, group_transition_seconds: 0 });
    const legacy = rebuild(tree);
    for (const day of legacy.days) for (const slot of day.exercises) {
    ...
    tree = await current(f.userId);
    expect(tree.days[0]!.exercises[0]!.group_id).toBe(f.groupId);
    const split = rebuild(tree);
    [split.days[0]!.exercises[1], split.days[0]!.exercises[2]] = [split.days[0]!.exercises[2]!, split.days[0]!.exercises[1]!];
    const before = await counts(f.userId);
    expect(await updatePlanTree(env.DB, f.userId, split)).toMatchObject({ error: 'group_conflict' });
    expect(await counts(f.userId)).toEqual(before);
    const removal = rebuild(tree); removal.days[0]!.exercises.splice(0, 1);
    expect(await updatePlanTree(env.DB, f.userId, removal)).toHaveProperty('plan');
    expect((await current(f.userId)).days[0]!.exercises[0]!.group_id).toBeNull();
  });

  it('rejects singleton and cross-day group payloads before creating a plan document', async () => {
    const f = await fixture(); const before = await counts(f.userId);
    const payload = rebuild(f.tree);
    Object.assign(payload.days[0]!.exercises[0]!, { group_id: f.groupId, group_rest_seconds: 30, group_transition_seconds: 0 });
    expect(await updatePlanTree(env.DB, f.userId, payload)).toMatchObject({ error: 'group_conflict' });
    payload.days[0]!.exercises[1] = { ...payload.days[0]!.exercises[1]!, group_id: f.groupId, group_rest_seconds: 30, group_transition_seconds: 0 };
    payload.days[1]!.exercises = [0, 1].map(() => ({ ...payload.days[0]!.exercises[0]! }));
  ...
  it('restores grouped snapshots and treats old absent group columns as ungrouped', async () => {
    ...
    for (const day of oldSnapshot.days) for (const slot of day.exercises) {
      delete slot.group_id; delete slot.group_rest_seconds; delete slot.group_transition_seconds;
    }
  ...
  it('validates prescriptions during grouping and clearing and keeps adjustments in lockstep', async () => {
    ...
    expect((await current(f.userId)).days[0]!.exercises.slice(0, 2).map((slot) => slot.target_sets)).toEqual([2, 2]);
  ...
  it('deletes a whole grouped day and preserves other days', async () => {
    const f = await fixture();
    await setGroup(env.DB, f.userId, f.day.id, f.groupId, f.ids.slice(0, 2), { expected_version: f.tree.version, round_rest: 30 });
    expect(await deleteDayTemplate(env.DB, f.userId, f.day.id, f.tree.version + 1)).toMatchObject({ ok: true });
    const tree = await current(f.userId);
    expect(tree.days.map((day) => day.name)).toEqual(['B']);
    expect(tree.days[0]!.exercises[0]!.group_id).toBeNull();
  });
  ...
  it('rejects orphan rest attributes in a full rebuild without discarding the invalid input', async () => {
    ...
    payload.days[0]!.exercises[0]!.group_rest_seconds = 45;
    expect(await updatePlanTree(env.DB, f.userId, payload)).toMatchObject({ error: 'group_conflict', fields: ['group_id'] });
    expect(await counts(f.userId)).toEqual(before);
  });
  ...
  it('keeps scoped clear receipts retryable after day removal without permitting a wrong-day clear', async () => {
    ...
    expect(await clearGroup(env.DB, f.userId, f.groupId, version, attrs, f.tree.days[1]!.id))
      .toMatchObject({ error: 'group_conflict' });
    const cleared = await clearGroup(env.DB, f.userId, f.groupId, version, attrs, f.day.id);
    await deleteDayTemplate(env.DB, f.userId, f.day.id, version + 1);
    const before = await counts(f.userId);
  ...
  it('requires an expected version for explicit group fields and every rebuild of a grouped plan', async () => {
    ...
    Object.assign(unversioned.days[0]!.exercises[0]!, { group_id: f.groupId, group_rest_seconds: 30, group_transition_seconds: 0 });
    Object.assign(unversioned.days[0]!.exercises[1]!, { group_id: f.groupId, group_rest_seconds: 30, group_transition_seconds: 0 });
    ...
    await setGroup(env.DB, f.userId, f.day.id, f.groupId, f.ids.slice(0, 2), { expected_version: f.tree.version, round_rest: 30 });
    const before = await counts(f.userId);
    for (const day of unversioned.days) for (const slot of day.exercises) {
    ...
    expect(await updatePlanTree(env.DB, f.userId, unversioned)).toEqual({ error: 'invalid_fields', fields: ['expected_version'] });
    expect(await updatePlanTree(env.DB, f.userId, { days: [] })).toEqual({ error: 'invalid_fields', fields: ['expected_version'] });
    expect(await counts(f.userId)).toEqual(before);
  });
